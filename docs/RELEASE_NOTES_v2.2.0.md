...
这是 Mineradio Next 独立仓库的首个发布候选版本。版本号从原项目 `2.1.0` 提升到
`2.2.0`，用于区分 Next 的品牌、功能与后续更新通道。

## 主要变化

- 完成 Mineradio Next 名称、音符 Logo、任务栏、托盘、快捷方式和安装器品牌统一。
- 保留原 Mineradio 用户数据目录，升级后账号、设置、歌单和缓存路径无需迁移。
- 整合音乐发现、平台排行、音乐电台、歌手专辑、伴奏查找和多来源搜索。
- 增加本地曲库、离线收藏、统一收藏、播放历史、歌单整理与导入工作流。
- 增加迷你播放器、系统媒体控制、托盘播放控制、定时停播和局域网遥控。
- 增加播放器实时频谱、前级增益、空间听感、独立倍速与音调调节。
- 完善完整桌面模式、Wallpaper Engine、本地视觉内容、桌面歌词和场景留影。
